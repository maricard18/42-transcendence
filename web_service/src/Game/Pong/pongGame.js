...
import { keys, ScreenHeight, ScreenWidth, updateVariables } from "./variables";
import { findTournamentWinner } from "../../views/Tournament";
import { gameConfettiAnimation, gameStartAnimation } from "./animations";
import { closeWebsocket, GameWebsocket, multiplayerPongMessageHandler, sendMessage } from "../../functions/websocket";
import { checkInvertedPlayer3Collision, checkInvertedPlayer4Collision, checkPlayer1Collision, checkPlayer2Collision } from "./collision";
import { createMultiPlayer2GameObjects, createMultiPlayer4GameObjects, createSinglePlayerGameObjects, createTournamentGameObjects } from "./createPlayers";
    ...
    return new Promise((resolve) => {
        const playPong = () => {
            if (game.over || !GameWebsocket.ws || !localStorage.getItem("game_status") ||
                !location.pathname.startsWith("/home/pong/play") || GameWebsocket.ws.readyState !== 1) {
                game.over = true;
                updateScore(game);
                resolve();
            } else if (!game.paused && !game.over && !game.lost_connection) {
                let current_time = Date.now();
                game.dt = (current_time - game.last_time) / 1000;
