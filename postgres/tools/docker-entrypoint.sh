...
export POSTGRES_DB=$(vault read /"$VAULT_MOUNT"/data/postgres-db -format=yaml | yq .data.data.key)
export POSTGRES_USER=$(vault read /"$VAULT_MOUNT"/data/postgres-user -format=yaml | yq .data.data.key)
export POSTGRES_PASSWORD=$(vault read /"$VAULT_MOUNT"/data/postgres-password -format=yaml | yq .data.data.key)

###########################################################################################
# docker-entrypoint.sh provided by Docker Official Image packaging for Postgres.
# https://github.com/docker-library/postgres/blob/master/13/alpine3.18/docker-entrypoint.sh
